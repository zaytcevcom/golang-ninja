...
import (
	...
	"github.com/zaytcevcom/golang-ninja/internal/store/predicate"
...
func (pu *ProblemUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	_spec := sqlgraph.NewUpdateSpec(problem.Table, problem.Columns, sqlgraph.NewFieldSpec(problem.FieldID, field.TypeOther))
	...
	if value, ok := pu.mutation.ManagerID(); ok {
		_spec.SetField(problem.FieldManagerID, field.TypeOther, value)
	}
	if pu.mutation.ManagerIDCleared() {
		_spec.ClearField(problem.FieldManagerID, field.TypeOther)
	}
	...
	if pu.mutation.ChatCleared() {
		edge := &sqlgraph.EdgeSpec{
			...
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(chat.FieldID, field.TypeOther),
			},
		}
	...
	if pu.mutation.MessagesCleared() {
		edge := &sqlgraph.EdgeSpec{
			...
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(message.FieldID, field.TypeOther),
			},
		}
		_spec.Edges.Clear = append(_spec.Edges.Clear, edge)
	}
