...
func (cu *ChatUpdate) sqlSave(ctx context.Context) (n int, err error) {
	_spec := sqlgraph.NewUpdateSpec(chat.Table, chat.Columns, sqlgraph.NewFieldSpec(chat.FieldID, field.TypeOther))
	...
	if value, ok := cu.mutation.ClientID(); ok {
		_spec.SetField(chat.FieldClientID, field.TypeOther, value)
	}
